...
import pty 
import logging
import yaml # For validator
import json # For validator/log history

# --- Logging Setup (same as orchestrator) ---
def setup_logging():
    ...
    # Orchestrator still logs to file, UI gets logs via socket
...
# --- Real-time Runner Logic (from app.py, slightly adapted) ---
def run_pipeline_thread(playbook_file="playbooks/remediate_compromised_host.yml"):
    ...
    try:
        ...
        # --- Trigger Method: Use docker-compose run directly ---
        # This simplifies things for the UI, directly running the orchestrator for the selected playbook
        command = [
            'docker-compose', 
            'run', '--rm', # Use run and remove container after
            'orchestrator', 
            'python3', 'orchestrator.py', 
            playbook_file # Pass the selected playbook
        ]
...
def run_playbook_endpoint():
    ...
    # Pass the selected playbook to the thread
...
# --- Validator Logic (Backend - Mirrors JS version) ---
# In production, policies might load from a DB/API
SAFETY_POLICIES = {
...
def validate_playbook_endpoint():
    """Validates playbook YAML syntax and safety policies."""
...
@app.route('/audit_log')
def get_audit_log():
    """Serves the content of the audit log file."""
...
def get_playbooks_list():
    """Returns a list of available playbook files."""
    ...
    try:
        ...
        # Return full path relative to project root for runner
...
if __name__ == '__main__':
    setup_logging()
    # Need PyYAML for validation: pip install PyYAML
